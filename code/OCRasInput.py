...
def main():
    ...
    with open(log_file_path, "w") as log_file:
        ...
        # Process each text file in the OCR folder of each subfolder
        for folder_name in os.listdir(data_path):
            subfolder_path = os.path.join(data_path, folder_name)
            if os.path.isdir(subfolder_path):  # Check if it's a directory
                print(f"Processing subfolder: {subfolder_path}")
                ocr_folder_path = os.path.join(subfolder_path, ocr_folder_name)
                ...
                if os.path.isdir(ocr_folder_path):  # Check if the OCR folder exists
                    print(f"Processing OCR folder: {ocr_folder_path}")
                    for file_name in os.listdir(ocr_folder_path):
                        if file_name.endswith(".txt"):  # Process .txt files
                            file_path = os.path.join(ocr_folder_path, file_name)
                            json_data = process_text_file(file_path)

                            # Create an output file in the OCRoutput folder
                            if json_data:

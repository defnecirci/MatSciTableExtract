...
def calculate_f1_scoreswithexcludingtables(data, ground_truth):
    ...
    all_FP = []
    all_FN = []
    any_comparable = False  # Flag to track if there's any comparable sample

    # First pass: Check if there's any comparable sample
    for data_item, truth_item in zip(data, ground_truth):
        if 'properties' in data_item and 'properties' in truth_item:
            any_comparable = True
            break

    # Only proceed if there are comparable samples
    if any_comparable:
        for data_item, truth_item in zip(data, ground_truth):
            if 'properties' in data_item and 'properties' in truth_item:
                sample_f1, matched, unmatched_data, unmatched_truth, TP, FP, FN = calculate_sample_f1(data_item['properties'], truth_item['properties'])
            else:
                # Assign an F1 score of 0 if 'properties' is missing in OCR output, but only if there are comparable samples
                sample_f1, matched, unmatched_data, unmatched_truth, TP, FP, FN = 0, [], [], list(truth_item['properties'].keys()), 0, 0, len(truth_item['properties'])
...
for folder_name in folder_names:
    ...
    for ground_truth_file, predicted_file in matched_files:
        ...
        if data is not None and ground_truth is not None:
            ...
            sample_f1_scores, average_f1, all_matched, all_unmatched_data, all_unmatched_truth, all_TP, all_FP, all_FN = calculate_f1_scoreswithexcludingtables(data, ground_truth)
            print("F1 Scores for each sample:", sample_f1_scores)
            print("Average F1 Score for this folder:", average_f1)
            Table_Accuracies.append(average_f1)
